...
for filename in filenames:
	...
	for i in range(nchains):		
		...
		chains[i].append('TER    ' + str(n) + aa.group()[0:3].rjust(9) + 
	...
	for i in range(nchains):
		...
		for j in range(len(hetatm[i])):
			...
			entry_res = re.search(r'NAP [A-Z]\s+\d+', hetatm[i][j])

			# replace residue number and entry number
			complete.append(hetatm[i][j].replace(entry_res.group(), 
				entry_res.group()[0:5] + str(res_new).rjust(4)).replace(entry_num.group(), 
